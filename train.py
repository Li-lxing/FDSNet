...
from losses import Fusion_loss, decomp_loss
import torch
...
def main():
    ...
    if args.distributed:
        model = FDSNet()
        model.cuda()
        FDSNet_model = nn.parallel.DistributedDataParallel(model,device_ids=[args.gpu],find_unused_parameters=True)
    else:
        FDSNet_model = FDSNet()
    FDSNet_model.cuda()
    ...
    if args.resume:
        FDSNet_model, optimizer.optim, ep, total_it = resume(FDSNet_model, optimizer.optim, args.resume)
        optimizer = Optimizer(
            model = FDSNet_model,
            lr0 = lr_start,
            momentum = momentum,
...
def trainer(train_loader,  FDSNet_model, seg_model, optimizer, opt, logger, start_ep=0, total_it=0,args=None,train_sampler=None):
    ...
    for ep in range(start_ep, total_epoch):
        ...
        for img_vi, vi_Y, _, _, img_ir,mask, _ in train_tqdm:
            ...
            fused_img,vis_ploss,ir_ploss, feature_V_B, feature_I_B, feature_V_D, feature_I_D = FDSNet_model(img_vi, img_ir,vis_mid_out_res,ir_mid_out_res,H,W,C)
            optimizer.zero_grad()
            
            # 计算特征分解约束损失
            loss_decomp_val, cc_loss_B_val, cc_loss_D_val = decomp_loss(feature_V_B, feature_I_B, feature_V_D, feature_I_D)
            
            # 解析损失权重，添加特征分解损失权重
            loss_weights = eval(opt.loss_weight)
            # 如果权重数量不足4个，添加特征分解损失的默认权重
            if len(loss_weights) < 4:
                loss_weights = list(loss_weights) + [2.0]  # 默认权重为2.0
            
            fusion_loss,prio_loss, grad_loss, int_loss, decomp_loss_val, cc_loss_B_val, cc_loss_D_val = Fusion_loss(
                vi_Y, img_ir, fused_img,vis_ploss,ir_ploss, mask=mask, weights=loss_weights, 
                decomp_loss_val=loss_decomp_val, cc_loss_B=cc_loss_B_val, cc_loss_D=cc_loss_D_val)
            
            train_tqdm.set_postfix(epoch=ep,
                                   prior_loss = loss_weights[0] * prio_loss.item(),
                                   gradinet_loss=loss_weights[1] * grad_loss.item(),
                                   int_loss=loss_weights[2] * int_loss.item(),
                                   decomp_loss=loss_weights[3] * decomp_loss_val.item(),
                                   cc_loss_B=cc_loss_B_val.item(),
                                   cc_loss_D=cc_loss_D_val.item(),
                                   loss_total=fusion_loss.item())
            fusion_loss.backward()
            optimizer.step()

        if ep % t == 0:
            saver.write_model(ep, total_it, FDSNet_model, optimizer.optim, is_best=False)
