...
def main():
    ...
    FDSNet_model = FDSNet().cuda()
    FDSNet_model = resume(FDSNet_model, model_save_path=opts.resume, is_train=False)
    ...
    tester(test_loader, FDSNet_model, opts, seg_model)


def tester(test_loader, FDSNet_model, opts, seg_model):
    FDSNet_model.eval()
    ...
    with torch.no_grad():
        ...
        for it, (img_ir, img_vi, img_names, widths, heights) in enumerate(test_bar):
            ...
            fused_img,vis_ploss,ir_ploss, _, _, _, _ = FDSNet_model(img_vi, img_ir, vis_mid_out_res, ir_mid_out_res,H,W,C)
